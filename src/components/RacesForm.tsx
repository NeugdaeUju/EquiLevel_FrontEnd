...
function RacesForm({ handleSubmit, setRacesName, racesName} : FormProps) {
    ...
    return (
        <form onSubmit={handleSubmit} className='race-form'>
            ...
            <select className='race-form__input' required value={selectedEspeceId ?? ''}
            onChange={(e) => setSelectedEspeceId(Number(e.target.value))}>
                <option className='race-form__option' value='' disabled>Choisissez une race</option>
                {especes.map(espece => (
                    <option key={espece.id} value={espece.id}>{espece.name}</option>
                ))}
            </select>
